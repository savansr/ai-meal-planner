...
export default clerkMiddleware(async (auth, req) => {
  // Use the auth object passed as first parameter
  ...
  // If it's the check-subscription route or create-profile route, skip logic to avoid loops
  if (pathname === "/api/check-subscription" || pathname === "/api/create-profile") {
    return NextResponse.next();
  }

  // If route is NOT public & user not signed in → redirect to /sign-up
  if (!isPublicRoute(req) && !userId) {
    console.log("Redirecting to sign-up: not authenticated for protected route");
    return NextResponse.redirect(new URL("/sign-up", origin));
  }

  // If user is signed in and visits /sign-up → redirect to mealplan
  if (isSignUpRoute(req) && userId) {
    console.log("Redirecting to mealplan: user already signed in");
  ...
  if ((isMealPlanRoute(req) || isProfileRoute(req)) && userId) {
    try {
      ...
      if (checkSubRes.ok) {
        ...
        // handle error
        console.log("Subscription check failed, redirecting to subscribe page");
        return NextResponse.redirect(new URL("/subscribe", origin));
      }
    } catch (error) {
